...
namespace NavMesh {
	...
	void PathFinder::AddExternalPoints(const std::vector<li::WorldPos>& points_)
	{
	...
	std::vector<li::WorldPos> PathFinder::GetPath(const li::WorldPos& start_coord, const li::WorldPos& dest_coord)
	{
		auto tmp = GetPath(
			li::WorldPosToPoint(start_coord),
			li::WorldPosToPoint(dest_coord));

		std::vector<li::WorldPos> ret;
		for (auto& it : tmp) ret.push_back(li::PointToWorldPos(it));
		return ret;
	}
